...
namespace OhControl.Radio
{
    ...
    public sealed class AtisService
    {
        ...
        public AtisBroadcast Build(TelemetrySnapshot telemetry)
        {
            ...
            int windDirection = RoundToNearest10(telemetry.WindDirectionTrueDeg);
            int windSpeed = Math.Max(0, (int)Math.Round(telemetry.WindSpeedKt));
            int qnh = telemetry.SeaLevelPressureMb > 800
                ? (int)Math.Round(telemetry.SeaLevelPressureMb)
                : 1013;

            int temperature = (int)Math.Round(telemetry.AmbientTemperatureC);
            int visibilityKm = telemetry.VisibilityMeters > 0
                ? Math.Max(1, (int)Math.Round(telemetry.VisibilityMeters / 1000.0))
                : 10;

            string runway = SelectRunway(windDirection, windSpeed);
        ...
        private static string SelectRunway(int windDirection, int windSpeed)
        {
            ...
            double differenceTo16 = AngularDifference(windDirection, 160);
            double differenceTo34 = AngularDifference(windDirection, 340);

            return differenceTo16 <= differenceTo34 ? "16" : "34";
        }
        ...
        private static int RoundToNearest10(double value)
        {
            ...
            int rounded = (int)(Math.Round(value / 10.0) * 10.0);
            return rounded == 360 ? 0 : rounded;
        }

        private static TelemetrySnapshot CreateFallbackTelemetry()
        {
